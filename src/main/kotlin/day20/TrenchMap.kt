    ...
    fun enhance(): Image {
        val enhanced = (minX + 1 until maxX).flatMap { x ->
            (minY + 1 until maxY).map { y ->
                ...
                pixel to TrenchMap.algorithm[newValue]
            }
        }.toMap()
...
object TrenchMap : Runnable {

    internal val algorithm = PuzzleData.load("/day20/algorithm.txt") { parseAlgorithm(it) }
    internal val image = PuzzleData.load("/day20/image.txt") { parseImage(it) }
